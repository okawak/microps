...
struct ip_iface *ip_iface_alloc(const char *unicast, const char *netmask) {
  ...
  if (ip_addr_pton(unicast, &iface->unicast) == -1) {
    errorf("ip_addr_pron() failure, addr=%s", unicast);
    memory_free(iface);
    return NULL;
  }
  if (ip_addr_pton(netmask, &iface->netmask) == -1) {
    errorf("ip_addr_pron() failure, addr=%s", netmask);
    memory_free(iface);
    return NULL;
  }
...
static void ip_input(const uint8_t *data, size_t len, struct net_device *dev) {
  ...
  if (v != IP_VERSION_IPV4) {
    errorf("invalid version error: v=%u", v);
    return;
  }
  hlen = (hdr->vhl & 0x0f) << 2;
  if (len < hlen) {
  ...
  if (offset & IP_HDR_FLAG_MF || offset & IP_HDR_OFFSET_MASK) {
    errorf("fragments does not supported");
    return;
  }
...
static ssize_t ip_build_packet(uint8_t protocol, const uint8_t *data,
                               size_t len, uint16_t id, uint16_t offset,
                               ip_addr_t src, ip_addr_t dst, uint8_t *buf,
                               size_t size) {
  uint16_t hlen, total;
  struct ip_hdr *hdr;
...
ssize_t ip_output(uint8_t protocol, const uint8_t *data, size_t len,
  ...
  if (!iface) {
    errorf("iface not found: %s", addr1);
    return -1;
  }
  ...
  id = random16();
  plen = ip_build_packet(protocol, data, len, id, 0, iface->unicast, dst, buf,
                         sizeof(buf));
  if (plen == -1) {
